/**
 * Vaultria — Static TTS Service v4
 *
 * Static-only lesson audio.
 *
 * Primary path:
 *   audio/manifest.json maps "lang::text" → "audio/{lang}/{hash}.wav"
 *   All lesson / phrase playback should resolve through this manifest.
 *
 * There is NO live network TTS fallback in this version.
 * If a clip is missing from the static pack, the app emits a warning event so
 * the content pack can be regenerated, instead of silently falling back.
 */
...
async function _getManifest() {
  ...
  _manifestLoad = fetch(MANIFEST_URL)
    ...
    .then(data => {
      ...
      console.log(`[TTS] manifest ok — ${keys.length} static entries loaded`);
      if (keys.length) console.log(`[TTS] manifest sample keys:`, keys.slice(0, 5));
      return _manifest;
    })
    .catch(err => {
      console.warn(`[TTS] manifest failed → ${err.message}`);
      _manifest = {};
      eventBus.emit("tts:missing-pack", { reason: "manifest", error: err.message, url: MANIFEST_URL });
      return _manifest;
    });
...
export async function speak(text, langKey, opts = {}) {
  ...
  // Same clip already playing — don't cut it off just to restart the same sound.
  ...
  if (!ready) {
    eventBus.emit("tts:missing-audio", { text, langKey, key, relativePath, reason: "load-failed" });
    return null;
  }
  ...
  try {
    ...
    eventBus.emit("tts:missing-audio", { text, langKey, key, relativePath, reason: "play-failed", error: err.message });
    if (_currentAudio === audio) _currentAudio = null;
    if (_currentKey === key) _currentKey = null;
    return null;
  }

  if (opts.shadowing) _scheduleShadowRepeat({ text, key, url, relativePath });
  return audio;
}
...
async function _resolveStaticAudio(ttsText, langCode) {
  ...
  if (!relativePath) {
    ...
    eventBus.emit("tts:missing-audio", { text: ttsText, langCode, key, reason: "manifest-miss" });
    return null;
  }

  console.log(`[TTS] static path chosen → key="${key}" file="${relativePath}"`);
  return { key, relativePath, url: _toAbsoluteAssetUrl(relativePath) };
}
...
function _readyAudio(audio, relativePath, key) {
  return new Promise(resolve => {
    ...
    const onError = () => {
      ...
      eventBus.emit("tts:missing-audio", { key, relativePath, reason: "decode-error" });
      done(false);
    };
    const cleanup = () => {
...
function _stopAudio() {
  if (_currentAudio) {
    _currentAudio.pause();
    try { _currentAudio.currentTime = 0; } catch {}
    _currentAudio = null;
    _currentKey = null;
  }
}
